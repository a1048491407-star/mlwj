...
function filterResponses(responses, query) {
  ...
  return responses.filter((item) => {
    const matchesSearch = !search || [item.id, item.name, item.phone, item.email].some((value) => String(value || "").toLowerCase().includes(search));
...
function toCsv(responses) {
  const headers = ["ID", "姓名", "电话", "电子邮件", "1. 投资经验", "2. 风险承受能力", "3. 可投资资本", "4. 期望利润", "5. 是否有时间监控股市", "6. 家人朋友是否知情", "7. 是否有不良记录", "提交时间", "状态"];
